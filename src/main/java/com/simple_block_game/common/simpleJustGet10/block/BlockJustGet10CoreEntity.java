...
public class BlockJustGet10CoreEntity extends BaseGameBlockEntity {
    ...
    protected void loadAdditional(@NonNull CompoundTag pTag, HolderLookup.@NonNull Provider pRegistries) {
        ...
        gameState = JustGet10GameState.fromSerializedName(tag.getString(KEY_STATE));
        score = tag.getInt(KEY_SCORE);
        int maxVal = tag.getInt(KEY_MAX);
        maxValue = ValueJustGet10.getByValue(maxVal).orElse(ValueJustGet10.NUM_1);
    }
}
